...
class SalonChair(models.Model):
    ...
    def create(self, values):
        ...
        for vals in values:
            ...
            if 'user_line' in vals:
                if vals['user_line']:
                    date_changer = []
                    for elements in vals['user_line']:
                        if len(elements) > 2 and isinstance(elements[2], dict) and 'start_date' in elements[2]:
                    ...
                    for elements in vals['user_line']:
                        ...
                        if len(vals['user_line']) == number:
                            break
                        if len(elements) > 2 and isinstance(elements[2], dict):
                            elements[2]['end_date'] = date_changer[number]
                    last_element = vals['user_line'][-1]
                    if len(last_element) > 2 and isinstance(last_element[2], dict):
                        vals['user_id'] = last_element[2].get('user_id', False)
                        vals['date'] = last_element[2].get('start_date', False)
