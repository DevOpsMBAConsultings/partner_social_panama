...
class SalonOrder(models.Model):
    ...
    stage_id = fields.Many2one(comodel_name='salon.stage', string="State",
                               ...
                               default=lambda self: self.env.ref('salon_management.salon_stage_00', raise_if_not_found=False).id if self.env.ref('salon_management.salon_stage_00', raise_if_not_found=False) else False,
    ...
    def write(self, values):
        ...
        # Get XML IDs for comparison
        stage_00 = self.env.ref('salon_management.salon_stage_00', raise_if_not_found=False)
        stage_01 = self.env.ref('salon_management.salon_stage_01', raise_if_not_found=False)
        stage_02 = self.env.ref('salon_management.salon_stage_02', raise_if_not_found=False)
        stage_03 = self.env.ref('salon_management.salon_stage_03', raise_if_not_found=False)
        stage_04 = self.env.ref('salon_management.salon_stage_04', raise_if_not_found=False)
        
        if 'stage_id' in values.keys():
            if stage_02 and self.stage_id.id == stage_02.id:
                if stage_03 and values['stage_id'] != stage_03.id:
            ...
            if stage_03 and self.stage_id.id == stage_03.id:
                raise ValidationError(
                    _("You can't move a salon order from Closed stage!"))
            if stage_04 and self.stage_id.id == stage_04.id:
                raise ValidationError(
                    _("You can't move a salon order from Cancel stage!"))
            if stage_00 and self.stage_id.id == stage_00.id:
                if (stage_01 and stage_04) and values['stage_id'] not in [stage_01.id, stage_04.id]:
                    raise ValidationError(_("You can't perform that move!"))
            if stage_01 and self.stage_id.id == stage_01.id:
                if stage_04 and values['stage_id'] == stage_04.id:
                    time_taken_chair = self.chair_id.total_time_taken_chair
                    self.chair_id.total_time_taken_chair = (
                            time_taken_chair - self.time_taken_total)
                elif (stage_00 and stage_03) and (values['stage_id'] == stage_00.id or values['stage_id'] == stage_03.id):
                    raise ValidationError(_("You can't perform that move!"))
                elif stage_02 and values['stage_id'] == stage_02.id and not self.inv_stage_identifier:
                    self.action_create_invoice()
        if 'stage_id' in values.keys() and self.name == "Draft Salon Order":
            if stage_01 and values['stage_id'] == stage_01.id:
                self.action_validate()
                self.action_confirm()
        res = super(SalonOrder, self).write(values)
    ...
    def action_confirm(self):
        ...
        stage_01 = self.env.ref('salon_management.salon_stage_01', raise_if_not_found=False)
        self.stage_id = stage_01.id if stage_01 else False
        self.update_number_of_orders()
    ...
    def action_close(self):
        ...
        stage_03 = self.env.ref('salon_management.salon_stage_03', raise_if_not_found=False)
        self.stage_id = stage_03.id if stage_03 else False
        self.update_number_of_orders()

    def action_cancel(self):
        """Function to cancel the salon order"""
        stage_04 = self.env.ref('salon_management.salon_stage_04', raise_if_not_found=False)
        self.stage_id = stage_04.id if stage_04 else False
        self.update_number_of_orders()
    ...
    def action_create_invoice(self):
        ...
        stage_02 = self.env.ref('salon_management.salon_stage_02', raise_if_not_found=False)
        stage_03 = self.env.ref('salon_management.salon_stage_03', raise_if_not_found=False)
        self.stage_id = stage_02.id if stage_02 else False
        
        valid_stages = []
        if stage_02: valid_stages.append(stage_02.id)
        if stage_03: valid_stages.append(stage_03.id)
            
        invoiced_records = self.env['salon.order'].search(
            [('stage_id', 'in', valid_stages), ('chair_id', '=', self.chair_id.id)])
        total = 0
    ...
    def unlink(self):
        ...
        stage_03 = self.env.ref('salon_management.salon_stage_03', raise_if_not_found=False)
        for order in self:
            if (stage_02 and order.stage_id.id == stage_02.id) or (stage_03 and order.stage_id.id == stage_03.id):
